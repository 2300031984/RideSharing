...
const ManageContacts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editingContact) {
        response = await EmergencyService.updateEmergencyContact(user.id, editingContact.id, formData);
      } else {
        response = await EmergencyService.addEmergencyContact(user.id, formData);
      }
  ...
  const handleDelete = async (contactId) => {
    if (window.confirm('Are you sure you want to delete this contact?')) {
      try {
        const response = await EmergencyService.deleteEmergencyContact(user.id, contactId);
        if (response.success) {
  ...
  const handleEmergencyCall = () => {
    // Call all primary contacts and emergency services
    const primaryContacts = contacts.filter(c => c.isPrimary);
    const emergencyServices = contacts.filter(c => c.isEmergencyService);
    
    if (primaryContacts.length > 0) {
      // Call the first primary contact
      window.open(`tel:${primaryContacts[0].contactNumber}`, '_self');
    } else if (emergencyServices.length > 0) {
      // Call the first emergency service
      window.open(`tel:${emergencyServices[0].contactNumber}`, '_self');
    } else {
    ...
    contacts.forEach((contact, index) => {
      if (contact.contactNumber) {
        setTimeout(() => {
          const smsUrl = EmergencyService.generateSMSUrl(contact.contactNumber, message);
          window.open(smsUrl, '_self');
        }, index * 1000); // Stagger the SMS sends
      }
    });
    
    setToast({ message: 'Emergency SMS sent to all contacts!', type: 'success' });
  };
      ...
      <Card style={{ marginBottom: 24, padding: 16, background: '#fef2f2', border: '2px solid #fecaca' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: '#7f1d1d', marginBottom: 16 }}>
            Quick emergency actions for all contacts
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                padding: '12px 20px',
                background: contacts.length > 0 ? '#dc2626' : '#9ca3af',
                color: 'white',
                border: 'none',
                borderRadius: 8,
            ...
            <button
              ...
              style={{
                padding: '12px 20px',
                background: contacts.length > 0 ? '#2563eb' : '#9ca3af',
                color: 'white',
                border: 'none',
                borderRadius: 8,
                cursor: contacts.length > 0 ? 'pointer' : 'not-allowed',
